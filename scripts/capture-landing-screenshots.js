#!/usr/bin/env node
/**
 * Hochauflösende Landingpage-Screenshots für CharcuLogicOS und HofSync.
 *
 * Aufruf:
 *   npm run screenshots:landing
 *   node scripts/capture-landing-screenshots.js
 *
 * CharcuLogicOS: lokaler Dev-Server (http://127.0.0.1:5173), sonst Auto-Start.
 * HofSync: Live-URL https://hofsync-production.web.app/ (Fallback: lokal mit HofSync-Branding).
 */
'use strict';
...
const CHARCULOGIC_SHOT = {
  file: 'charculogic.png',
  url: `${LOCAL_ORIGIN.replace(/\/$/, '')}/index.html`,
  branding: {
...
const HOFSYNC_SHOT = {
  ...
  fallbackUrl: `${LOCAL_ORIGIN.replace(/\/$/, '')}/index.html`,
  branding: {
    appName: 'HofSync',
...
async function dismissOverlays(page) {
  await page.evaluate(() => {
    try {
      sessionStorage.removeItem('charculogic_prefer_phone_shell');
    } catch (_) { /* noop */ }
    ...
    const saveBar = document.querySelector('#page-mhd .sticky-action-bar');
...
async function prepareDashboard(page) {
  await page.evaluate(() => {
    document.body.classList.remove('is-dev-dashboard', 'dev-dashboard-view');
    document.body.classList.add('app-shell-sidebar', 'has-admin-nav');

    const navBrand = document.getElementById('app-nav-brand');
    if (navBrand) navBrand.setAttribute('aria-hidden', 'false');
    ...
    const enabledTabs = new Set(['mhd', 'receiving', 'chargenDoku', 'kitchen', 'haccp']);
    ...
    if (hint) hint.textContent = 'MHD in den kommenden 21 Tagen · 4 Karten';
      ...
      <div class="mhd-card status-warning">
        <div class="mhd-card-header">
          <div class="mhd-product-info">
            ...
            <span class="mhd-product-meta">Eigenproduktion · MHD 20.08.2026 · 1 aktiver Posten</span>
          </div>
      ...
      <div class="mhd-card status-ok">
        <div class="mhd-card-header">
          <div class="mhd-product-info">
            ...
            <span class="mhd-product-meta">Demeter · MHD 23.08.2026 · 3 aktive Posten</span>
          </div>
      ...
      <div class="mhd-card status-ok">
        <div class="mhd-card-header">
          <div class="mhd-product-info">
            ...
            <span class="mhd-product-meta">MoPro · MHD 28.08.2026 · 1 aktiver Posten</span>
          </div>
          <div class="mhd-badge" style="color:#2E7D32;background:rgba(46,125,50,0.14);">10 Tage</div>
        </div>
        <div class="mhd-controls-row">
          <div class="qty-stepper">
            <button class="btn-stepper" type="button">−</button>
            <div class="qty-value-container"><span>6</span></div>
            <button class="btn-stepper" type="button">+</button>
          </div>
          <button class="btn btn-soldout" type="button">🗑️ Ausverkauft</button>
        </div>
...
async function captureShot(browser, shot) {
  ...
  await page.waitForSelector('.app-nav-zone--ops', { timeout: 10000 });
  ...
  log(`   ${outPath}  (${Math.round(stat.size / 1024)} KB, ${VIEWPORT.width * DEVICE_SCALE_FACTOR}×${VIEWPORT.height * DEVICE_SCALE_FACTOR} px)`);
  return outPath;
}
