...
import { reconcileDeliveryNote, showReconcileOverlay, removeReconcileOverlay } from './delivery-reconcile.js';
import {
  buildDeliveryParserWriteOps,
  createDeliveryParserBatchId,
  toMhdKategorie,
} from './delivery-parser-writes.js';
...
async function schreibeLieferposten(row, { author, nowIso, batchId, index, tenantId }) {
  const writeFn = parserState.writeOrQueueFirestore;
  if (typeof writeFn !== 'function') {
    throw new Error('Speichern ist noch nicht bereit. Bitte Seite neu laden.');
  }

  const writeOps = buildDeliveryParserWriteOps(row, {
    tenantId,
    author,
    nowIso,
    batchId,
...
async function bucheLieferungEin(rows) {
  ...
  const author = authContext.email?.split('@')[0] || 'Team';
  const tenantId = parserState.tenantId || authContext.tenantId || '';
  const nowIso = new Date().toISOString();
  const batchId = createDeliveryParserBatchId();
  ...
  try {
    ...
    for (let index = 0; index < rows.length; index += 1) {
      ...
      const result = await schreibeLieferposten(row, {
        author,
        nowIso,
        batchId,
        index,
    ...
    window.showToast?.('Lieferschein erfolgreich verbucht. Bestand und MHD-Posten sind gespeichert.', 'success');
  } catch (err) {
