...
describe('customer order pickup stock integrity', () => {
  ...
  it('allows paper-slip placeholder orders without stock deduction', async () => {
    const order = {
      id: 'order-slip',
      status: 'ready',
      items: [{ product: 'Siehe Bestellzettel (Scan)', quantity: '1', unit: 'Aufnahme' }],
      orderSlipAttachments: [{ url: 'https://example.invalid/slip.jpg', type: 'image' }],
      tenantId: TENANT_ID,
    };
    const { store } = createHarness({ order });

    const deductions = await __customerOrdersTest.prepareStockDeductionsForOrder(order);
    expect(deductions).to.have.length(0);

    const deducted = await __customerOrdersTest.markOrderPickedUpWithStock(order, 'Team');
    expect(deducted).to.equal(0);
    expect(store.get(`tenants/${TENANT_ID}/customerOrders/order-slip`).status).to.equal('picked_up');
  });
